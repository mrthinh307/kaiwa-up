...
from app.schemas.reflex import ReviewScheduleItem
from app.services.reflex import ReflexService

router = APIRouter(prefix="/review", tags=["Review"])


@router.get("/due", operation_id="listDueReviews", response_model=list[ReviewScheduleItem])
async def list_due_reviews(
    ...
    return await ReflexService(ReflexRepository(session), ai_gateway).list_schedules(
        user_id=current_user.id, is_due_only=True
    )


@router.get("/schedule", operation_id="listReviewSchedule", response_model=list[ReviewScheduleItem])
