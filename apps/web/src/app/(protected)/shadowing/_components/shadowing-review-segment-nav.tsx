...
type ShadowingSegmentState = "correct" | "incorrect" | "not_started" | "pending" | "unavailable";

function getShadowingSegmentState(segment: ShadowingSegmentReviewItem): ShadowingSegmentState {
  if (!segment.recorded) {
    return "not_started";
  }
  if (
    segment.transcription_status === "failed" ||
    segment.transcription_status === "unavailable" ||
    segment.transcription_status === "not_evaluable"
  ) {
    return "unavailable";
  }
  const score = segment.text_match_score ?? segment.similarity_score;
  if (score !== null && score !== undefined) {
    return score >= 80 ? "correct" : "incorrect";
  }
  return "pending";
}
...
const STATE_CONFIG = {
  pending: {
    ...
    bgClass: "border-border bg-background text-foreground/70",
    icon: Clock,
    iconColor: "text-foreground/70",
    label: "Awaiting comparison",
    legendBadge: "border-border bg-background",
  },
  unavailable: {
    badgeClass: "border-border bg-secondary-background text-foreground",
    bgClass: "border-border bg-secondary-background text-foreground",
    icon: AlertCircle,
    iconColor: "text-foreground",
    label: "Comparison unavailable",
    legendBadge: "border-border bg-secondary-background",
  },
  correct: {
    badgeClass:
...
const LEGEND_STATES: readonly ShadowingSegmentState[] = [
  "correct",
  "incorrect",
  "pending",
  "unavailable",
...
type ShadowingReviewSegmentNavProps = {
  activeSegmentIndex: number;
  onSelectSegment: (index: number) => void;
  segments: ShadowingSegmentReviewItem[];
};

export function ShadowingReviewSegmentNav({
  activeSegmentIndex,
  onSelectSegment,
  segments,
}: ShadowingReviewSegmentNavProps) {
  ...
  return (
    <section
      aria-labelledby="shadowing-segments-heading"
      className="rounded-base border-2 border-border bg-secondary-background p-3.5 shadow-shadow sm:p-4"
    >
      {/* Header with Title, Page Indicator, Active Segment, and Show all segments toggle */}
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h2 className="font-heading text-sm sm:text-base" id="shadowing-segments-heading">
            Segment map
          </h2>
          {shouldCollapseMap && !isMapExpanded ? (
        ...
        <div className="flex items-center gap-2">
          {shouldCollapseMap ? (
            <Button
              aria-controls="shadowing-segment-grid"
      ...
      {/* Segment Buttons Row (Collapsed) or Grid (Expanded) */}
      {!isMapExpanded ? (
        <div className="mt-3 flex items-center gap-1.5 sm:gap-2">
          ...
          <div
            ...
            id="shadowing-segment-grid"
            ref={segmentGridRef}
          >
            {visibleSegments.map((segment) => {
              const state = getShadowingSegmentState(segment);
              const config = STATE_CONFIG[state];
              const Icon = config.icon;
              const isActive = segment.segment_index === activeSegmentIndex;
      ...
      <ul className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-border/40 pt-2.5 text-xs sm:gap-5">
